...
ISR(TIMER1_OVF_vect)
{
  ...
  int heatAdc = analogRead(heatSensor);
  int coolAdc = analogRead(coolSensor);
  int floatAdc = analogRead(floatSensor);
  int switchesAdc = analogRead(switches);
  int overAdc = analogRead(overSensor);
  int doorAdc = analogRead(door);
  ...
  verbose(heatAdc, coolAdc, floatAdc, switchesAdc, overAdc, doorAdc);
}

void verbose(heatAdc, coolAdc, floatAdc, switchesAdc, overAdc, doorAdc)
{
